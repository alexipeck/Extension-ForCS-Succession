...
namespace Landis.Extension.Succession.ForC
{
        ...
        : Landis.Library.Succession.ExtensionBase
    {
        ...
        public override void Run()
        {
            ...
            // This state only survives the current timestep and most of these dictionaries are cleared after iterating through every site
            var speciesLookup = new Dictionary<ISpecies, ISpeciesCohorts>();
            var ageLookup = new Dictionary<ISpecies, Dictionary<ushort, ICohort>>();
            var concreteLookup = new Dictionary<ISpecies, SpeciesCohorts>();
                    ...
                    foreach (ISpeciesCohorts speciesCohorts in siteCohorts) {
                        ...
                        foreach (ICohort cohort in speciesCohorts) {
                            if (speciesName == "pinubank" || speciesName == "querelli") {
                                modelCore.UI.WriteLine($"Site: ({site.Location.Row},{site.Location.Column}), Species: {speciesName}, Age: {cohort.Data.Age}, Biomass: {cohort.Data.Biomass}");
                            }
                        }
                    }
                }
                        ...
                        foreach (KeyValuePair<ushort, int> ageEntry in speciesEntry.Value) {
                            ...
                            if (ageLookup[targetSpecies].TryGetValue(age, out var targetCohort)) {
                                ...
                                if (concreteLookup.TryGetValue(targetSpecies, out var concreteCohorts)) {
                                    concreteCohorts.AddNewCohort(age, transfer, new System.Dynamic.ExpandoObject());
                                } else {
